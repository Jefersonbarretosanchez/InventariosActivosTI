...
function TablaCatSOBack() {
  ...
  const [sistemaOperativoSeleccionado, setSistemaOperativoSeleccionado] = useState(null);
  ...
  const fetchSistemasOperativos = async () => {
    setIsLoading(true);
    try {
      const response = await api.get(`${API_URL}/api/so/`);
      setSistemasOperativos(response.data);
    } catch (error) {
      toast.error("Error cargando los sistemas operativos");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleInputChange = (event) => {
    ...
    setNewSOData((prevData) => ({ ...prevData, [name]: value }));
  };
  ...
  const createSO = async () => {
    ...
    try {
      ...
      const response = await api.post(`${API_URL}/api/so/`, formattedData);
      const nuevoSO = response.data;
      setSistemasOperativos([...sistemasOperativos, nuevoSO]);
      setNewSOData({});
      cambiarEstadoModal(false);
      toast.success("Sistema operativo creado exitosamente!");
    } catch (error) {
  ...
  const updateSO = async () => {
    ...
    try {
      ...
      const response = await api.put(
        `${API_URL}/api/so/${sistemaOperativoSeleccionado.id_so}/`,
        formattedData
      );
      const updatedSO = response.data;
      setSistemasOperativos(
        sistemasOperativos.map((so) =>
          so.id_so === updatedSO.id_so
            ? updatedSO
            : so
        )
      );
      setNewSOData({});
      cambiarEstadoModal(false);
      toast.success("Sistema operativo actualizado exitosamente!");
    } catch (error) {
      const errorMessage = error.response
        ? error.response.data.message
        : error.message;
  ...
  const handleCreate = () => {
    abrirModal("Agregar Sistema Operativo", formFields, [], {}, "create");
  };

  const handleEdit = (so) => {
    setSistemaOperativoSeleccionado(so);
    abrirModal(
      `Editar ${so.nombre}`,
      formFields,
      ["id"],
  ...
  const filteredSistemasOperativos = sistemasOperativos.filter((so) => {
    const searchString = `${so.id_so} ${so.nombre} ${so.fecha_registro}`.toLowerCase();
    const matchesSearch = searchString.includes(searchTerm.toLowerCase());

    const matchesFilters = Object.keys(filtroValues).every((key) => {
      if (!filtroValues[key]) return true;
      return String(so[key]) === String(filtroValues[key]);
    });
  ...
  const currentRecords = filteredSistemasOperativos.slice(
    indexOfFirstRecord,
    indexOfLastRecord
  );
  const totalPages = Math.ceil(filteredSistemasOperativos.length / recordsPerPage);
      ...
      <div className="contenedor-activos">
        <div className="row-activos">
          ...
          <div className="contenedor-principal">
            <div style={{ marginLeft: '18.5vw' }} className="contbuscador-personas">
              <input
            ...
            <table style={{ width: "100%" }} className="table-personas">
              ...
              <tbody>
                {isLoading ? (
                  ...
                  currentRecords.map((so) => (
                    <tr key={so.id_so}>
                      <td style={{ paddingLeft: '7.5vw' }}>{so.id_so}</td>
                      <td>{so.nombre}</td>
