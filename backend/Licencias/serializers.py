...
class AsignacionLicenciasPersonasSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields= ['id','id_trabajador','nombre_trabajador', 'id_licencia','nombre_licencia']
        
    def validate(self, data):
        if self.instance is None:
            # Comprobar si la licencia ya está asignada
            asignacion_existente = AsignacionLicenciaPersona.objects.filter(id_licencia=data['id_licencia']).first()
            ...
            trabajador_asignacion = AsignacionLicenciaPersona.objects.filter(id_trabajador=data['id_trabajador']).first()
            ...
            if 'id_trabajador' in data and self.instance.id_licencia != data['id_licencia']:
                ...
                if asignacion_existente:
                    ...
                    raise serializers.ValidationError(
                        f"La licencia ya está asignada a {usuario.nombres} {usuario.apellidos} (ID: {usuario.id_trabajador}, Email: {usuario.correo_institucional})."
                    )
                trabajador_asignacion = AsignacionLicenciaPersona.objects.filter(id_trabajador=data['id_trabajador']).first()
                if trabajador_asignacion:
                    licencia = trabajador_asignacion.id_licencia
    ...
    def create(self, validated_data):
        ...
        lic_persona.id_estado_licencia = CatEstadoLicencias.objects.get(nombre="Asignada")
        lic_persona.save()
...
class DesAsignacionLicenciasPersonaSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        if 'id_licencia' in validated_data:
            lic_persona = validated_data['id_licencia']
            lic_persona.id_estado_licencia = CatEstadoLicencias.objects.get(nombre="Sin Asignar")
            lic_persona.save()
...
class AsignacionLicenciasEquiposSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        if self.instance is None:
            # Comprobar si la licencia ya está asignada
            asignacion_existente = AsignacionLicenciasEquipo.objects.filter(id_licencia=data['id_licencia']).first()
            if asignacion_existente:
            ...
            equipo_asignacion = AsignacionLicenciasEquipo.objects.filter(id_equipo=data['id_equipo']).first()
            ...
            if 'id_equipo' in data and self.instance.id_licencia != data['id_licencia']:
                ...
                if asignacion_existente:
                    equipo = asignacion_existente.id_equipo
                    raise serializers.ValidationError(
                        f"La licencia ya está asignada a {equipo.nombre_equipo} (ID: {equipo.id_equipo})."
                    )
                equipo_asignacion = AsignacionLicenciasEquipo.objects.filter(id_equipo=data['id_equipo']).first()
                if equipo_asignacion:
                    licencia = equipo_asignacion.id_licencia
    ...
    def create(self, validated_data):
        ...
        equipo.id_estado_licencia = CatEstadoLicencias.objects.get(nombre="Asignada")
        equipo.save()
        return super().create(validated_data)
...
class DesAsignacionLicEquSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        if 'id_licencia' in validated_data:
            ...
            equipo.id_estado_licencia = CatEstadoLicencias.objects.get(nombre="Sin Asignar")
            equipo.save()
        return super().update(instance, validated_data)